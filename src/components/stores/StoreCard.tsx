...
export default function StoreCard({ store }: StoreCardProps) {
  ...
  return (
    <article className="group flex h-full flex-col justify-between overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all duration-300 hover:shadow-md dark:border-slate-800 dark:bg-slate-900">
      ...
      <div>
        ...
        {/* Products Thumbnail Grid or Empty State */}
        {hasProducts ? (
          ...
          <div className="mt-2 flex h-[92px] w-full flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50/50 p-2 text-center dark:border-slate-800 dark:bg-slate-800/20">
            <p className="text-[11px] text-slate-400">No featured products yet</p>
            <p className="text-[10px] text-slate-400">Check store page for updates</p>
          </div>
        )}
      </div>

      {/* Card Footer (Always pushed to the bottom) */}
      <div className="mt-4 flex items-center justify-between pt-3 border-t border-slate-100 dark:border-slate-800">
        ...
        <Link
          ...
          className="rounded-lg bg-blue-600 px-3.5 py-1.5 text-xs font-semibold text-white shadow-md shadow-blue-600/20 transition hover:bg-blue-700"
        >
          Visit Store →
        </Link>
      </div>
