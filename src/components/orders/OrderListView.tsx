...
interface OrderListViewProps {
  basePath?: string; // e.g. "/orders" or "/dashboard/user/orders"
...
export function OrderListView({
  ...
  const returnedOrdersCount = orders.filter(
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      {/* Optional AI Assistant Quick Bar */}
      {showAiBanner && (
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-bold text-foreground">
                Need quick answers about your orders, returns, or delivery?
              </p>
              <p className="text-[11px] text-muted">
                Ask ShopNest AI to check return windows, track courier riders, or summarize purchases.
              </p>
            </div>
          </div>
          <Link
      ...
      {/* Optional Order Summary Quick Cards */}
      {showDashboardStats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <button
            type="button"
            onClick={() => setStatusFilter("all")}
            className={`rounded-2xl border p-4 text-left sm:text-center cursor-pointer transition ${
              statusFilter === "all"
