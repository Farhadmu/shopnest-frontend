...
export interface OrderDetailViewProps {
  orderId: string;
  backHref?: string; // default "/orders" or "/dashboard/user/orders"
...
export function OrderDetailView({
  ...
  const loadOrder = useCallback(async () => {
    ...
    try {
      ...
      try {
        const trackingData = await getDeliveryTracking(orderId);
  ...
  const handleReturn = async () => {
    if (!order) return;
    const reason = prompt(
      "Please enter the reason for returning this item (e.g. Size mismatch, defective, wrong color):"
    );
  ...
  const handleRiderRateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setActionSuccessMsg("Thank you! Your delivery partner rating has been recorded.");
    } catch (err) {
      alert(err instanceof Error ? err.message : "Failed to submit rider rating.");
    } finally {
  ...
  if (loading && !order) {
    return (
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center space-y-4">
        ...
        <h2 className="text-lg font-bold text-foreground">Synchronizing Order & Dispatch Data...</h2>
        <p className="text-xs text-muted">Retrieving courier telemetry, package contents, and payment confirmation.</p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-xl mx-auto px-4 py-16 text-center">
        <div className="bg-card border border-border rounded-3xl p-8 shadow-sm space-y-4">
          ...
          <h2 className="text-xl font-extrabold text-foreground">Order Access Unavailable</h2>
          <p className="text-xs text-muted leading-relaxed">
            {error || "The requested order could not be located or you may not have permission to view it."}
          </p>
          <div className="pt-2">
            <Link
              href={backHref}
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5 text-xs text-muted">
                  ...
                  <span className="capitalize">
                    {tracking.assignedRider.vehicleType} ({tracking.assignedRider.vehicleModel || "Vehicle"})
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-muted text-[11px]">
                    Coordinates: {tracking.currentLocation.latitude.toFixed(4)}° N,{" "}
                    {tracking.currentLocation.longitude.toFixed(4)}° E
                    {tracking.currentLocation.updatedAt &&
                      ` • Updated ${new Date(tracking.currentLocation.updatedAt).toLocaleTimeString()}`}
                  </p>
                </div>
              </div>
          ...
          {TRACKING_STEPS.map((step, idx) => {
            ...
            return (
              <div key={step.key} className="flex flex-col items-center text-center relative group">
                ...
                <span className="text-[11px] text-muted mt-0.5 leading-tight">{step.desc}</span>
              </div>
            );
          })}
        </div>
      ...
      {/* Main Grid: Purchased Items & Financial Details */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left: Products & Delivery Details */}
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-card border border-border rounded-2xl p-6 shadow-sm">
            <h2 className="text-base font-bold text-foreground mb-4 flex items-center gap-2">
              <FiPackage className="text-primary text-lg" /> Purchased Products ({order.items.length})
            </h2>
                    ...
                    <div className="flex items-center gap-2">
                      {order.status !== "cancelled" && (
                        <button
                          ...
                          onClick={() =>
                            setReviewTarget({
                              id: item.productId || (item as { id?: string }).id || (item as any)._id || "",
                              title: item.title || `Product #${index + 1}`,
                            })
                          }
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-500/10 text-amber-600 dark:text-amber-400 hover:bg-amber-500/20 border border-amber-500/30 rounded-xl text-xs font-bold transition-colors cursor-pointer"
                        >
                          <FiStar className="fill-amber-400 text-amber-400" /> Write Review
                        </button>
                      )}
                      {order.status === "delivered" && (
                        <Link
                          href={`/dashboard/user/returns/new?orderId=${String(order.id || (order as any)._id || "")}&productId=${item.productId}`}
                          className="inline-flex items-center gap-1 px-3 py-1.5 bg-primary/10 text-primary rounded-xl text-xs font-bold hover:bg-primary hover:text-white transition cursor-pointer"
                        >
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="flex justify-between text-muted">
                ...
                <span className="text-foreground font-semibold">
                  ৳{(order.deliveryFee ?? 60).toLocaleString()}
                </span>
              </div>

              <div className="border-t border-border pt-3 flex justify-between items-center text-sm font-black text-foreground">
                <span>Total Amount</span>
                <span className="text-primary text-base">৳{order.totalAmount.toLocaleString()}</span>
              </div>
            </div>

            <div className="mt-6 p-4 rounded-xl bg-muted-bg/60 border border-border/80 flex items-start gap-3">
