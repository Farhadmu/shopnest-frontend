  ...
  return (
    <div className="min-h-[calc(100vh-9rem)] py-2">
      <div className="grid gap-6 lg:grid-cols-[250px_1fr]">
        <aside className="hidden rounded-2xl border border-border bg-surface p-3 shadow-sm lg:block lg:sticky lg:top-24 lg:h-fit">
          ...
          <nav className="grid gap-1">
            {links.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="group rounded-xl p-3 transition hover:bg-muted-bg"
              >
        ...
        <main className="min-w-0">
          <div className="mb-5 rounded-2xl border border-border bg-surface p-5 shadow-sm sm:p-7">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              ...
              <Link
                href="/products"
                className="rounded-xl border border-border bg-background px-4 py-2.5 text-sm font-bold text-text transition hover:border-primary/40 hover:text-primary"
              >
                Continue shopping →
              </Link>
            </div>
      ...
      {links.map((item, index) => (
        ...
        >
          ...
          >
            ...
            <span className="mt-4 inline-flex text-xs font-bold text-primary">Open feature →</span>
          </Link>
        </motion.div>
      ))}
    </div>
