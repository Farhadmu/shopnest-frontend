  ...
  return (
    <div className="grid gap-5">
      <div>
        ...
        <h1 className="mt-1 text-3xl font-black">Smart Inventory</h1>
        <p className="mt-2 text-sm text-muted">
          Monitor stock health and jump directly to product editing.
        </p>
      </div>
          ...
          <tbody>
            {products.map((p) => (
              <tr key={p.id} className="border-b border-border last:border-0">
                ...
                <td
                  className={`p-4 font-bold ${p.stock <= 0 ? "text-error" : p.stock <= 5 ? "text-warning" : "text-success"}`}
                >
                  {p.stock}
                </td>
                <td className="p-4">
                  <a className="font-bold text-primary" href={`/seller/products?edit=${p.id}`}>
                    Manage →
                  </a>
                </td>
              </tr>
