...
export default function AdminUsers() {
  const [users, setUsers] = useState<Array<{ id: string; name: string; email: string; role: string; banned: boolean }>>([]);
  const [search, setSearch] = useState("");
  const load = () => {
    clientFetch<Array<{ id: string; name: string; email: string; role: string; banned: boolean }>>("/users", { params: { search: search || undefined } })
      .then((r) => setUsers((r as { data?: Array<{ id: string; name: string; email: string; role: string; banned: boolean }> }).data ?? r))
      .catch(() => setUsers([]));
  };
