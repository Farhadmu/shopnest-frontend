...
export default function AdminOrdersPage() {
  ...
  const loadOrders = () => {
    setLoading(true);
    clientFetch<AdminOrder[]>("/orders/admin/all")
      .then((r) => setOrders((r as { data?: AdminOrder[] }).data ?? r ?? []))
      .catch(() => setOrders([]))
              ...
              >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted mt-0.5">
                        Customer ID: {(o.userId ?? "").slice(-6) || "N/A"} • Placed {new Date(o.createdAt ?? Date.now()).toLocaleString()}
                      </p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                  <div className="p-3 bg-muted-bg/50 rounded-xl border border-border/40">
                    ...
                    <p className="text-foreground font-medium">{o.shippingAddress || "N/A"}</p>
                    <p className="text-muted mt-1">Payment: <strong className="text-foreground uppercase">{o.paymentMethod || "COD"}</strong></p>
                  </div>

                  <div className="p-3 bg-muted-bg/50 rounded-xl border border-border/40">
                    <p className="font-semibold text-muted mb-1">Purchased Products ({Array.isArray(o.items) ? o.items.length : 0})</p>
                    <div className="space-y-1">
                      {(Array.isArray(o.items) ? o.items : []).map((it: { title?: string; quantity: number; price: number }, idx: number) => (
