...
export function SpendingAnalyticsClient({
  ...
  const hasSpendingData = Boolean(
    analytics && ((analytics.totalSpent ?? 0) > 0 || (analytics.completedOrders ?? 0) > 0)
  );

  const totalNetSpend = analytics?.totalSpent ?? 0;
  ...
  const averageOrderValue =
    ...
    (completedOrders > 0 ? Math.round(totalNetSpend / completedOrders) : 0);
  ...
  const monthlyData: MonthPoint[] = cadenceCheckpoints.map((slot, idx) => {
    ...
    if (isCurrent && totalNetSpend > 0) {
      return {
        ...
        orders: completedOrders > 0 ? completedOrders : 1,
      };
    }
  ...
  const maxWeeklyFromApi = analytics?.weeklySpending?.length
    ? Math.max(...analytics.weeklySpending.map((x) => x.amount))
    : 0;

  const weeklyData: WeekDayPoint[] =
    hasSpendingData && analytics?.weeklySpending && analytics.weeklySpending.length === 7
      ? analytics.weeklySpending.map((w) => ({
          day: w.day,
          amount: w.amount,
          isPeak: maxWeeklyFromApi > 0 && w.amount === maxWeeklyFromApi,
        }))
      : defaultWeekly;

  const peakWeeklyItem = weeklyData.find((w) => w.isPeak && w.amount > 0);
  const peakDayName = peakWeeklyItem ? peakWeeklyItem.day : null;
  const maxWeeklyAmount = Math.max(...weeklyData.map((w) => w.amount), 1);

  // Category distribution
  const totalCategoryAmount = (analytics?.categorySpending ?? []).reduce(
    (sum, c) => sum + (c.amount || 0),
    0
  );

  const categoryList =
    hasSpendingData && analytics?.categorySpending && analytics.categorySpending.length > 0
      ? analytics.categorySpending.map((c, i) => {
          const safePercentage =
            totalCategoryAmount > 0
              ? Math.min(100, Math.max(0, Math.round((c.amount / totalCategoryAmount) * 100)))
              : Math.min(100, Math.max(0, c.percentage || 0));
  ...
  // Vendors & Stores
  const vendorList =
    hasSpendingData && analytics?.sellerSpending && analytics.sellerSpending.length > 0
      ? analytics.sellerSpending.map((s, idx) => {
          const rawOrderId = (s as { latestOrderId?: string }).latestOrderId;
          const displayOrderId = rawOrderId
            ? (rawOrderId.length > 8 ? rawOrderId.slice(-8).toUpperCase() : rawOrderId)
            : s.sellerId
            ? s.sellerId.slice(-6).toUpperCase()
            : `${idx + 1000}`;
  ...
  const sparklinePoints = hasSpendingData
    ? [35, 42, 38, 55, 48, 62, 58, 75, 70, 85]
  ...
  const sparklineSvgPath = sparklinePoints
    .map((p, i) => {
      ...
      const y = sparklineMax === sparklineMin ? 20 : 36 - ((p - sparklineMin) / (sparklineMax - sparklineMin)) * 28;
  ...
  return (
    ...
    >
      <div className="space-y-6">
        {/* Error banner */}
        {error && (
          <div className="flex items-center gap-3 rounded-md border border-red-500/30 bg-red-500/10 p-4 text-sm font-semibold text-red-600 dark:text-red-400">
        ...
        <div className="flex flex-col gap-4 rounded-lg border border-[#E2E8F0] bg-white p-4 shadow-sm transition-all sm:flex-row sm:items-center sm:justify-between dark:border-[#2D2250] dark:bg-[#130E26]">
          {/* Left: Currency Switcher & Date Range Dropdown */}
          <div className="flex flex-wrap items-center gap-3">
            {/* Currency Pill Switcher */}
            <div className="flex items-center rounded-md border border-[#E2E8F0] bg-[#F8FAFC] p-1 dark:border-[#2D2250] dark:bg-[#090614]">
              <button
                type="button"
                onClick={() => setCurrency("BDT")}
                className={`flex items-center gap-1 rounded-md px-3 py-1.5 text-xs font-black transition-all ${
                  currency === "BDT"
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                <select
                  ...
                  className="appearance-none rounded-md border border-[#E2E8F0] bg-[#F8FAFC] py-2 pl-9 pr-9 text-xs font-bold text-[#0F172A] outline-none transition focus:border-[#4F46E5] dark:border-[#2D2250] dark:bg-[#090614] dark:text-[#F8FAFC]"
                >
            ...
            <div className="flex items-center gap-2 rounded-md border border-[#E2E8F0] bg-[#F8FAFC] px-3 py-1.5 dark:border-[#2D2250] dark:bg-[#090614]">
              <Checkbox
          ...
          <div className="flex items-center gap-2.5">
            <Button
              ...
              size="sm"
              variant="outline"
              className="flex items-center gap-1.5 rounded-md border-[#E2E8F0] text-xs font-black text-slate-800 hover:border-[#4F46E5] hover:text-[#4F46E5] dark:border-[#2D2250] dark:text-slate-200"
            >
            ...
            <Button
              ...
              size="sm"
              className="flex items-center gap-1.5 rounded-md bg-gradient-to-r from-[#4F46E5] to-[#7C3AED] text-xs font-black text-white shadow-md shadow-indigo-500/20 hover:opacity-95"
            >
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <Card className="rounded-lg border border-[#E2E8F0] bg-white p-5 shadow-sm transition-all hover:shadow-md dark:border-[#2D2250] dark:bg-[#130E26]">
              ...
              <div className="flex items-center justify-between">
                ...
                <Chip size="sm" className="bg-emerald-500/10 text-[10px] font-black text-emerald-500">
                  <span className="flex items-center gap-1">
                    <TrendingUp className="h-3 w-3" /> {hasSpendingData ? "+8.4%" : "0.0%"}
                  </span>
                </Chip>
              </div>
          ...
          <Card className="rounded-lg border border-[#E2E8F0] bg-white p-5 shadow-sm transition-all hover:shadow-md dark:border-[#2D2250] dark:bg-[#130E26]">
            <CardContent className="p-0">
              ...
              <div className="mt-2 flex items-center gap-1.5 text-xs font-bold text-slate-500 dark:text-slate-400">
                ...
                <span>{hasSpendingData ? "2.4 Days average delivery timeline" : "No delivery activity recorded"}</span>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-0">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] font-black text-[#7C3AED]">
                  {hasSpendingData ? "Steady Growth" : "No Orders"}
                </span>
              </div>
              <div className="mt-2 flex items-end justify-between">
                <div>
                  ...
                  <p className="mt-2 text-xs font-bold text-slate-500 dark:text-slate-400">
                    {hasSpendingData ? "Across completed purchases" : "No completed purchases"}
                  </p>
                </div>
                ...
                <div className="w-24 pb-1">
                  <svg viewBox="0 0 120 40" className="h-9 w-24 overflow-visible">
                    ...
                    />
                    <circle cx="120" cy="20" r="3.5" fill="#7C3AED" />
                  </svg>
                </div>
              </div>
            </CardContent>
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-12">
          ...
          <div className="xl:col-span-7">
            <Card className="h-full rounded-lg border border-[#E2E8F0] bg-white p-6 shadow-sm dark:border-[#2D2250] dark:bg-[#130E26]">
              <CardContent className="p-0">
                ...
                <div className="relative mt-4">
                  ...
                  {activePoint && (
                    ...
                    >
                      <div className="w-[245px] rounded-md border border-slate-700/80 bg-[#0B0F19] p-4 text-white shadow-2xl backdrop-blur-md dark:border-slate-700 dark:bg-[#080516]">
                        <div className="flex items-center gap-2">
          ...
          <div className="xl:col-span-5">
            <Card className="flex h-full flex-col justify-between rounded-lg border border-[#E2E8F0] bg-white p-6 shadow-sm dark:border-[#2D2250] dark:bg-[#130E26]">
              <CardContent className="flex h-full flex-col justify-between p-0">
                ...
                <div className="flex items-start justify-between gap-3 border-b border-[#E2E8F0] pb-4 dark:border-[#2D2250]">
                  ...
                  <span className="rounded-md bg-[#F5F3FF] px-3 py-1 text-xs font-semibold text-[#7C3AED] dark:bg-[#251A45] dark:text-indigo-400">
                    {peakDayName ? `Peak: ${peakDayName}` : "No Activity Yet"}
                  </span>
                </div>
                ...
                <div className="my-auto flex h-52 flex-col justify-end pt-8 pb-1">
                  ...
                  <div className="flex h-40 items-end justify-between gap-3 px-1">
                    {weeklyData.map((item) => {
                      const isPeak = Boolean(hasSpendingData && item.isPeak && item.amount > 0);
                      const isMidDay = Boolean(hasSpendingData && !isPeak && item.amount > 0);
                      const heightPercent = isPeak ? 85 : isMidDay ? 32 : 8;
                        ...
                        >
                          ...
                          <div
                            ...
                            className={`w-full cursor-pointer transition-all duration-300 ${
                              isPeak
                                ? "rounded-t-md bg-gradient-to-t from-[#4338CA] via-[#4F46E5] to-[#7C3AED] shadow-md shadow-indigo-500/20"
                                : isMidDay
                                ? "rounded-t-sm bg-[#C7D2FE] hover:bg-[#A5B4FC] dark:bg-[#3730A3]/70 dark:hover:bg-[#3730A3]"
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 pt-4 text-xs dark:border-slate-800/80">
                  <div className="text-slate-500 dark:text-slate-400">
                    ...
                    <span className="font-bold text-[#0F172A] dark:text-[#F8FAFC]">
                      {hasSpendingData ? "2:00 PM – 4:30 PM" : "—"}
                    </span>
                  </div>
                  <div className="flex items-center gap-1 font-bold text-slate-500 dark:text-slate-400">
                    {hasSpendingData ? (
                      <>
                        ...
                        <span className="text-emerald-600 dark:text-emerald-400">Midweek Shopper</span>
                      </>
                    ) : (
                      <span>No checkout activity</span>
                    )}
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="p-0">
                ...
                {categoryList.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    <div className="mb-3 flex h-14 w-14 items-center justify-center rounded-md bg-indigo-50 text-[#4F46E5] dark:bg-indigo-950/50">
                    ...
                    <p className="mt-1 max-w-xs text-xs text-slate-500 dark:text-slate-400">
                      Start shopping to see your analytics and merchandise category distribution.
                    </p>
                    <Link
                      href="/products"
                      className="mt-4 inline-flex items-center gap-1.5 rounded-md bg-[#4F46E5] px-4 py-2 text-xs font-bold text-white shadow-sm hover:bg-[#4338CA] transition-all"
                    >
                        ...
                        >
                          ...
                          <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                            <div
                              ...
                              style={{ width: `${Math.min(100, Math.max(0, cat.percentage))}%`, backgroundColor: cat.color }}
                            />
                          </div>
                        </div>
                      ))}
          ...
          <div className="xl:col-span-6">
            <Card className="h-full rounded-lg border border-[#E2E8F0] bg-white p-6 shadow-sm dark:border-[#2D2250] dark:bg-[#130E26]">
              <CardContent className="p-0">
                ...
                {vendorList.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    <div className="mb-3 flex h-14 w-14 items-center justify-center rounded-md bg-emerald-50 text-emerald-600 dark:bg-emerald-950/50 dark:text-emerald-400">
                    ...
                    <p className="mt-1 max-w-xs text-xs text-slate-500 dark:text-slate-400">
                      Start shopping to see your analytics and verified merchant performance.
                    </p>
                    <Link
                      href="/products"
                      className="mt-4 inline-flex items-center gap-1.5 rounded-md border border-[#E2E8F0] bg-white px-4 py-2 text-xs font-bold text-slate-800 shadow-sm hover:border-[#4F46E5] hover:text-[#4F46E5] transition-all dark:border-[#2D2250] dark:bg-[#130E26] dark:text-slate-200"
                    >
                      <ShoppingBag className="h-3.5 w-3.5 text-[#4F46E5]" />
                  ...
                  <div className="mt-6 space-y-3">
                    {vendorList.map((vendor, i) => (
                      <div
                        key={i}
                        className="group flex flex-col justify-between gap-3 rounded-md border border-[#E2E8F0] bg-[#F8FAFC] p-4 transition-all hover:border-[#4F46E5]/40 hover:shadow-xs sm:flex-row sm:items-center dark:border-[#2D2250] dark:bg-[#090614]"
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-md bg-gradient-to-tr ${vendor.gradient} text-sm font-black text-white shadow-sm`}
                          >
                            {vendor.initials}
                          </div>
                          <div>
                    ...
                    {/* Summary Callout */}
                    <div className="mt-4 flex items-center justify-between rounded-md border border-dashed border-[#E2E8F0] p-3 text-xs text-slate-500 dark:border-[#2D2250] dark:text-slate-400">
          ...
          <div className="xl:col-span-7">
            <Card className="h-full rounded-lg border border-[#E2E8F0] bg-white p-6 shadow-sm dark:border-[#2D2250] dark:bg-[#130E26]">
              <CardContent className="p-0">
                <div className="flex items-center justify-between border-b border-[#E2E8F0] pb-4 dark:border-[#2D2250]">
                  <div className="flex items-center gap-2">
                    <div className="flex h-7 w-7 items-center justify-center rounded-md bg-indigo-500/10 text-[#4F46E5]">
                  ...
                  <Chip size="sm" className="bg-[#4F46E5]/10 text-xs font-black text-[#4F46E5] dark:text-indigo-400">
                    {hasSpendingData ? "3 Active Alerts" : "System Ready"}
                  </Chip>
                </div>
                ...
                <div className="mt-5 space-y-3.5">
                  {hasSpendingData ? (
                    <>
                      ...
                      <div className="flex items-start gap-3.5 rounded-md border border-amber-500/20 bg-amber-500/5 p-4">
                        ...
                        <div>
                          <h4 className="text-xs font-black text-amber-800 dark:text-amber-300">
                            Category Concentration Warning ({categoryList[0]?.category || "Electronics"}: {categoryList[0]?.percentage || 62}%)
                          </h4>
                          <p className="mt-1 text-xs leading-relaxed text-slate-600 dark:text-slate-300">
                            High volume concentration in {categoryList[0]?.category || "Electronics"}. Consider exploring scheduled replenishment to unlock 5% bulk tier vendor discounts.
                          </p>
                        </div>
                      </div>

                      {/* Insight 2: VIP Tier Threshold Update */}
                      <div className="flex items-start gap-3.5 rounded-md border border-[#7C3AED]/20 bg-[#7C3AED]/5 p-4">
                        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-[#7C3AED]/10 text-[#7C3AED]">
                      ...
                      <div className="flex items-start gap-3.5 rounded-md border border-emerald-500/20 bg-emerald-500/5 p-4">
                    ...
                    <>
                      <div className="flex items-start gap-3.5 rounded-md border border-indigo-500/20 bg-indigo-500/5 p-4">
                        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-indigo-500/10 text-[#4F46E5]">
                          <Sparkles className="h-4 w-4" />
                        </div>
                        <div>
                          <h4 className="text-xs font-black text-[#4F46E5] dark:text-indigo-300">
                            Smart Spend Intelligence Ready
                          </h4>
                          <p className="mt-1 text-xs leading-relaxed text-slate-600 dark:text-slate-300">
                            Your automated expense tracking, category distribution, and delivery intelligence will activate right after your first order.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start gap-3.5 rounded-md border border-emerald-500/20 bg-emerald-500/5 p-4">
                        ...
                        <div>
                          <h4 className="text-xs font-black text-emerald-800 dark:text-emerald-300">
                            85% Spend Ceiling Guard Active
                          </h4>
                          <p className="mt-1 text-xs leading-relaxed text-slate-600 dark:text-slate-300">
                            You currently have a monthly budget target of {formatMoney(targetBudget)}. We will automatically warn you before checkout if a transaction approaches your limit.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start gap-3.5 rounded-md border border-[#7C3AED]/20 bg-[#7C3AED]/5 p-4">
                        ...
                        <div>
                          <h4 className="text-xs font-black text-[#7C3AED]">
                            VIP Tier Rewards Ready
                          </h4>
                          <p className="mt-1 text-xs leading-relaxed text-slate-600 dark:text-slate-300">
                            Purchases across verified stores unlock progressive tiers granting priority fulfillment, customer support, and fee-free deliveries.
                          </p>
                        </div>
                      </div>
                    </>
            ...
            <Card className="h-full rounded-lg border border-[#E2E8F0] bg-white p-6 shadow-sm dark:border-[#2D2250] dark:bg-[#130E26]">
              <CardContent className="p-0">
                ...
                <div className="mt-5 grid grid-cols-3 gap-2.5">
                  <div className="rounded-md border border-[#E2E8F0] bg-[#F8FAFC] p-3 text-center dark:border-[#2D2250] dark:bg-[#090614]">
                ...
                {isBudgetApproaching ? (
                  <div className="mt-5 flex items-start gap-2.5 rounded-md border border-amber-500/20 bg-amber-500/10 p-3 text-xs text-amber-800 dark:text-amber-300">
                  ...
                  <div className="mt-5 flex items-center gap-2 rounded-md border border-emerald-500/20 bg-emerald-500/10 p-3 text-xs text-emerald-800 dark:text-emerald-300">
      ...
      {showBudgetModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-xs">
          <div className="w-full max-w-md rounded-lg border border-[#E2E8F0] bg-white p-6 shadow-2xl dark:border-[#2D2250] dark:bg-[#130E26]">
            ...
            <form onSubmit={handleBudgetSave} className="mt-4 space-y-4">
              <div>
                ...
                <div className="relative mt-1">
                  <input
                    ...
                    className="w-full rounded-md border border-[#E2E8F0] bg-[#F8FAFC] px-4 py-2.5 text-sm font-black text-[#0F172A] outline-none transition focus:border-[#4F46E5] dark:border-[#2D2250] dark:bg-[#090614] dark:text-[#F8FAFC]"
                    autoFocus
                  />
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                <Button
                  ...
                  className="rounded-md text-xs font-bold text-slate-600 dark:text-slate-300"
                >
                  Cancel
                </Button>
                <Button
                  ...
                  className="rounded-md bg-gradient-to-r from-[#4F46E5] to-[#7C3AED] text-xs font-black text-white shadow-md"
                >
