...
export default function CustomerComplaintsPage() {
  ...
  const loadComplaints = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getCustomerComplaints({ page, limit: 20, status: filterStatus || undefined });
      setItems(res.items);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await createCustomerComplaint(form);
  ...
  return (
    <DashboardShell
      role="Customer"
      title="My Complaints"
      subtitle="Submit and track your complaints. Real data, unlimited submissions."
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`rounded-full px-2 py-0.5 text-[9px] font-black uppercase ${colors.badge}`}>{item.status}</span>
                        </div>
