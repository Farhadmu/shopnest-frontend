...
export default function DeliveryComplaintsPage() {
  ...
  const loadComplaints = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getDeliveryComplaints({ page, limit: 20, status: filterStatus || undefined });
      setItems(res.items);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await createDeliveryComplaint(form);
  ...
  const openDetail = async (id: string) => {
    setLoading(true);
    try {
      const detail = await getDeliveryComplaintById(id);
      setSelected(detail);
      setView("detail");
    } catch (error) {
  ...
  return (
    <DashboardShell
      role="Delivery Man"
      title="My Complaints"
      subtitle="Submit and track delivery-related complaints."
