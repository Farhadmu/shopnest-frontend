    ...
    try {
      ...
      if (result.error) {
        if (
          ...
          setErrorMsg("Your email is not verified yet. Please complete email verification before signing in.");
        } else {
      ...
      <div className="flex-1 min-h-0 flex flex-col lg:flex-row overflow-hidden">
        ...
        >
          <div className="w-full max-w-88 sm:max-w-[24rem] xl:max-w-100 my-auto flex flex-col justify-center">
            ...
            {isJustVerified && !errorMsg && (
              <motion.div
                initial={{ opacity: 0, y: -4, scale: 0.97 }}
                animate={{ opacity: 1, y: 0, scale: 1 }}
                role="status"
                className="mb-2 flex items-center gap-2 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-3 py-1.5 text-[11px] sm:text-xs font-semibold text-emerald-600 dark:text-emerald-400 shadow-sm"
              >
            ...
            <AnimatePresence mode="wait">
              {errorMsg && (
                <motion.div
                  ...
                  role="alert"
                  className="mb-2 flex flex-col gap-1 rounded-xl border border-rose-500/25 bg-rose-500/10 px-3 py-2 text-[11px] sm:text-xs font-medium text-rose-600 dark:text-rose-400 shadow-sm"
                >
                  ...
                  {unverifiedEmail && (
                    <div className="pl-5 pt-0.5">
                      <Link
                        href={`/verify-email?email=${encodeURIComponent(unverifiedEmail)}&next=${encodeURIComponent(nextParam || "/")}`}
